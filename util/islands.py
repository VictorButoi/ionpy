...
def dfs(
    ...
    : param visited: a numpy array tracking which 
...
def get_connected_components(
    ...
    :param label: A binary 2D numpy array.
    :return: A list of binary 2D numpy arrays, one for each connected component.
    """
    ...
    for i in range(rows):
        for j in range(cols):
            if not visited[i, j] and label[i, j]:
                ...
                connected_components.append(comp_image)
